...
evidence from AKOUO-style listening claims and marks a curated seed set as
...
def build_claims(prompt: dict, generation: dict, analysis: dict, review_status: str) -> dict:
    ...
    if review_status == "hybrid_reviewed":
        interpreted.append({
            ...
            "confidence": "medium",
            "basis": "AKOUO-style interpretive review of prompt adherence and procedural synthesis limits",
        })
        interpreted.append({
...
def provenance_for(score_sets: dict, review_status: str) -> list[dict]:
    ...
    for axis in SCORE_AXES:
        ...
        if axis in ("generic_naturalism_index", "cultural_cliche_index", "ecological_plausibility", "causal_coherence"):
            evidence = "AKOUO-style interpretive rubric plus prompt/metadata comparison"
        provenance.append({
            "axis": axis,
            "score": final[axis],
            "scorer": scorer,
...
def build_report(prompt: dict, generation: dict, analysis: dict, report_num: int) -> dict:
    ...
    return {
        ...
        "reviewer_notes": [
            "v0.1.1 QA pass combining signal inspection, prompt comparison, and AKOUO-style claim discipline.",
        ...
        "revision_history": [
            {
                ...
                "change": "v0.1.1 report regenerated with review status, score provenance, and populated AKOUO claim taxonomy.",
            }
        ],
        ...
        "sources": {
            "detected": layers,
            "inferred": [f"synthetic {category} control profile"],
            "absent_expected": prompt.get("intended_sources", []),
            "forbidden_detected": [],
            "hallucinated": ["synthesis artifacts standing in for requested sources"],
        },
        "spatial_structure": {
            "foreground": layers[-1],
            "midground": layers[0],
        ...
        "causal_coherence": (
            "Limited. Events are generated by synthesis rules rather than by modeled causes in a coherent environment."
        ),
