  ...
  return (
    <>
      <div className="page-header">
        <h1 className="page-title">{prompt.prompt_id}</h1>
        <p className="page-subtitle">
          ...
          {prompt.loop_required && <span className="badge badge-loop" style={{ marginLeft: 4 }}>loop required</span>}
        </p>
      </div>
      ...
      <div className="compare-grid">
        <div className="detail-section">
          ...
          <div className="card">
            {prompt.intended_sources.map((s) => <span key={s} className="source-tag source-intended">{s}</span>)}
          </div>
        </div>
        <div className="detail-section">
          ...
          <div className="card">
            {prompt.forbidden_sources.length > 0
              ? prompt.forbidden_sources.map((s) => <span key={s} className="source-tag source-forbidden">{s}</span>)
      ...
      <div className="detail-section">
        ...
        {generations.length > 0 ? (
          <div className="card" style={{ overflowX: "auto" }}>
            <table className="data-table">
              <thead><tr><th>Audio ID</th><th>Model</th><th>Type</th><th>Duration</th><th>Date</th></tr></thead>
              <tbody>
                {generations.map((g) => (
                  <tr key={g.audio_id}>
                    <td><Link href={`/generations/${g.audio_id}`} className="mono-link">{g.audio_id}</Link></td>
                    <td>{g.model}</td>
                    <td><span className="badge badge-control">{modelTypeLabel(g.model)}</span></td>
                    <td>{g.duration}s</td>
        ...
        ) : <div className="card"><p style={{ color: "var(--text-muted)" }}>No generations yet.</p></div>}
      </div>
      ...
      <div className="detail-section">
          ...
          <div className="card-grid">
            {reports.map((r) => (
              <div className="card" key={r.report_id}>
                <div className="card-title">{r.report_id} → {r.audio_id}</div>
                <div className="card-meta">{r.listener_type} · {r.review_status} · {r.listening_date}</div>
                <p style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 8 }}>{r.basic_description.slice(0, 150)}…</p>
        ...
        ) : <div className="card"><p style={{ color: "var(--text-muted)" }}>No reports yet.</p></div>}
      </div>
    </>
  );
}
