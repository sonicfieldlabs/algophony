...
The combined capacity of machine systems to generate speech, music, and soundscapes and to hear, transcribe, classify, and decode sound at scale. AAI gives the machine both a mouth and an ear; the manifesto's premise is that both are political.
...
Generation metadata recording the material footprint of a run: whether it executed locally or through a cloud API or hosted endpoint, on what hardware, in what region, with any available energy notes. It operationalizes the manifesto's planetary claim that every generated sound has a body somewhere: electricity, water, lithium, heat, labor.
